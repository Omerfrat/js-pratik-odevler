...
let carBox = document.querySelector('.car');

for (let i = 0; i < cars.length; i++) {
    carBox.innerHTML += `<td class='brand' data-index=${i}>${cars[i].brand}</td>`+ 
    `<td class='model' data-index=${i}>${cars[i].model}</td>` +
     `<td class='color' data-index=${i}>${cars[i].color}</td> ` +
     `<td class='image'><img src='${cars[i].image}' alt='${cars[i].brand} ${cars[i].model}' /></td>`;
}
